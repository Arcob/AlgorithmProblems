...
int longestSubstringWithoutDuplication_2(const std::string& str)
{
	int maxLength = 0;
	int tempLength = 0;
	int *lastPositionHash = new int[26];
	for (int i = 0; i < 26; i++) {
		lastPositionHash[i] = -1;
	}
	for (int i = 0; i < str.length(); i++) {
		int prevIndex = lastPositionHash[static_cast<int>(str[i] - 'a')];
		if (i - prevIndex > tempLength) {
			tempLength += 1;
			...
			tempLength = i-prevIndex; //不是改为0而是从上次出现的地方切断
		}
		lastPositionHash[static_cast<int>(str[i] - 'a')] = i;
	}
	delete[] lastPositionHash;
	return maxLength;
}
