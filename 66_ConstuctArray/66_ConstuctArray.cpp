// 66_ConstuctArray.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
// 面试题66：构建乘积数组
...
void BuildProductionArray(const vector<double>& input, vector<double>& output)
{
	...
	leftArray[0] = 1.0;
	rightArray[length - 1] = 1.0;
	for (int i = 1; i < length; i++) {
		leftArray[i] = leftArray[i - 1] * input[i - 1];
	}

	for (int i = length-2; i >=0; i--) {
		rightArray[i] = rightArray[i + 1] * input[i + 1];
	}

	for (int i = 0; i < length; i++) {
		output[i] = rightArray[i] * leftArray[i];
	}
}
...
static void test1()
{
	...
	vector<double> out = vector<double>(output, output + sizeof(output) / sizeof(double));
