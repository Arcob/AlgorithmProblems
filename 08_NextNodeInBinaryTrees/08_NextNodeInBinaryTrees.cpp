// 08_NextNodeInBinaryTrees.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
BinaryTreeNode* GetNext(BinaryTreeNode* pNode)
{
	...
	else if (pNode->m_pParent != nullptr && pNode == pNode->m_pParent->m_pLeft) {
		result = pNode->m_pParent;
	}
	else if (pNode->m_pParent != nullptr){
		BinaryTreeNode* temp = pNode->m_pParent;
		while (temp->m_pParent!=nullptr && temp != temp->m_pParent->m_pLeft)
		{
			temp = temp->m_pParent;
		}
		if (temp->m_pParent == nullptr) {
			return nullptr;
		}
		result = temp->m_pParent;
	}
	return result;
}
